// Inline SVG 圖示（Lucide 線條風）—— 攤位不依賴 CDN，離線也能顯示。
// 用法：
//   HTML 靜態： <svg class="ic"><use href="#ic-sparkles"></use></svg>
//   JS 動態：   icon("sparkles")            → 回傳上面那段字串
//   會轉的載入： icon("loader-circle","spin")→ 加 .ic-spin 動畫
// 顏色/描邊：都吃 CSS（.ic{stroke:currentColor;...}），所以會跟著文字色走。
(function () {
  // 每個 symbol 只放「內部路徑」；外框的 stroke/fill 交給 style.css 的 .ic 統一設定。
  var ICONS = {
  ...
  // 把所有 icon 塞成一張隱藏的 SVG sprite，注入到 document 最前面。
  var symbols = "";
  for (var name in ICONS) {
    symbols += '<symbol id="ic-' + name + '" viewBox="0 0 24 24">' + ICONS[name] + "</symbol>";
  }
  ...
  // JS 動態產生 icon 字串（給 app.js 用）。extra 可加 class，例如 "spin"。
  window.icon = function (name, extra) {
    return '<svg class="ic' + (extra ? " ic-" + extra : "") + '" aria-hidden="true"><use href="#ic-' + name + '"></use></svg>';
  };
})();
